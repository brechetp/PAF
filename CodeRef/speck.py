...
class SPECK:
    ...
    def encrypt(self, plaintext):
        ...
        print 'Initial L', hex(l)[2:-1].zfill(8)#+hex(r)[2:-1].zfill(8)
        print 'Initial R', hex(r)[2:-1].zfill(8)
        print ''
        for i in range(self.__num_rounds):
            l, r = self.__first_feistel(l, r)
            r ^= self.__round_key[i]
            l, r = self.__second_feistel(l, r)
            print 'Key in Round', i,'', hex(self.__round_key[i])[2:-1].zfill(8)
            print 'L in Round', i,'', hex(l)[2:-1].zfill(8)#+hex(r)[2:-1].zfill(8)
            print 'R in Round', i,'', hex(r)[2:-1].zfill(8)
            print ''
...
if __name__ == '__main__':
    ...
    plain=0x3b7265747475432d
    cipher=0x8c6fa548454e028b
    my_speck = SPECK(bsize, ksize, key)
    encrypted = my_speck.encrypt(plain)
    print 'Ciphertext', hex(encrypted)[2:-1].zfill(8)
